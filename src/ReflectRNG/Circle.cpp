...
std::vector<Point> Circle::getIntersections(Ray ray)
{
	...
	if ((end - ray.start).dot(center - ray.start) < 0) return {}; // Circle is not in the same direciton as the line.
	if (radius < abs(D)) return {};                               // Circle is too small to reach the line.
	if (radius == abs(D))                                         // Line is a tangent to the circle.
		return {
			{D * dy + center.x, -D * dx + center.y}
		};
